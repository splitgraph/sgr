...
### `FROM [remote URL] mountpoint[:tag] IMPORT table1 [AS table1_alias], [table2...]`

Uses the `sg import` command to import one or more tables from either a local mountpoint or a remote one. Currently
doesn't support importing from FDW-mounted databases.

The commit hash produced by this command is a combination of the current `OUTPUT` hash, the hash of the source
mountpoint and the hashes of the names and aliases of all imported tables. This is crude, but means that the layer is
invalidated if there's a change on the remote or we import a different table/name it differently. We can improve on this
by perhaps only considering the objects and table aliases that are actually imported (as opposed to the source image
hash: perhaps the tables we're importing haven't changed even if other parts of the mountpoint have).

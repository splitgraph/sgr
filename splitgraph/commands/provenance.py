...
def prov_command_to_sgfile(prov_type, prov_data, image_hash):
    """
    ...
    if prov_type == "IMPORT":
        prov_data = json.loads(prov_data)
        result = "FROM %s:%s IMPORT " % (prov_data['source'], prov_data['source_hash'])
        ...
        return "FROM %s:%s" % (prov_data, image_hash)
    elif prov_type == "SQL":
        return "SQL " + prov_data.replace("\n", "\\\n")
    raise SplitGraphException("Cannot reconstruct provenance %s!" % prov_type)


def image_hash_to_sgfile(conn, mountpoint, image_hash, err_on_end=True):
    """
    ...
    while image_hash:
        ...
        if prov_type in ('IMPORT', 'SQL', 'FROM'):
            sgfile_commands.append(prov_command_to_sgfile(prov_type, prov_data, image_hash))

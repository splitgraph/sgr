...
@click.option('-a', '--against', multiple=True, type=parse_image_spec,
              help='Images to substitute into the reconstructed Splitfile, of the form'
                   ' [NAMESPACE/]REPOSITORY[:HASH_OR_TAG].')
def rebuild_c(image_spec, update, against):
    ...
    Example: `sgr rebuild my_repo:latest --against noaa/climate:old_data` will reconstruct the Splitfile used to
    create `my_repo:latest`, replace all imports from `noaa/climate` with imports from `noaa/climate:old_data`
    and rerun the Splitfile.

...
def _table_to_df(engine, schema, table, **kwargs):
    return pd.read_sql_table(table_name=table, con=_get_sqlalchemy_engine(engine), schema=schema, **kwargs)


def _sql_to_df(engine, sql, schema, **kwargs):
    return pd.read_sql_query(
        sql=SQL("SET search_path TO {};").format(Identifier(schema)).as_string(engine.connection) + sql,
        con=_get_sqlalchemy_engine(engine), **kwargs)
...
def sql_to_df(sql, image=None, repository=None, use_lq=False, **kwargs):
    ...
    if image is None:
        ...
        return _sql_to_df(engine=repository.engine, sql=sql, schema=repository.to_schema(), **kwargs)
...
def _df_to_empty_table(engine, df, target_schema, target_table, use_ordinal_hack=False):
    ...
    if df.index.names == [None]:
        ...
        ddl = get_schema(df.reset_index(), name=target_table, keys=df.index.names, con=_get_sqlalchemy_engine(engine))
    ...
    if use_ordinal_hack:
        ...
        engine.run_sql_in(target_schema,
                          SQL("ALTER TABLE {0} DROP COLUMN {1};ALTER TABLE {0} ADD COLUMN {1} %s" % ctype)
                          .format(Identifier(target_table), Identifier(cname)))
...
def _df_to_table_fast(engine, df, target_schema, target_table):
    ...
    with engine.connection.cursor() as cur:
        ...
        buffer.seek(0)
        cur.copy_from(buffer, SQL("{}.{}").format(Identifier(target_schema), Identifier(target_table)).as_string(cur),
                      null="")
    engine.commit()
...
def df_to_table(df, repository, table, if_exists='patch'):
    ...
    try:
        ...
        if not _schema_compatible(source_schema, target_schema):
            raise SplitGraphException("Schema changes are unsupported with if_exists='patch'!"
        ...
        query = SQL("INSERT INTO {0}.{1} (SELECT * FROM {0}.{2})").format(Identifier(schema), Identifier(table),
                                                                          Identifier(tmp_table))
        query += SQL(" ON CONFLICT (") + SQL(',').join(map(Identifier, pk_cols)) + SQL(")")
        if non_pk_cols:
            if len(non_pk_cols) > 1:
                query += SQL(" DO UPDATE SET (") + SQL(',').join(map(Identifier, non_pk_cols)) + SQL(") = (") \
                         + SQL(',').join([SQL("EXCLUDED.") + Identifier(c) for c in non_pk_cols]) + SQL(")")
            else:

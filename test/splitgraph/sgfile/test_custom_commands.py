...
def test_dummy_command(sg_pg_conn):
    ...
    execute_commands(load_sgfile('custom_command_dummy.sgfile'), output=OUTPUT)
...
def test_calc_hash_short_circuit(sg_pg_conn):
    ...
    execute_commands(load_sgfile('custom_command_calc_hash.sgfile'), output=OUTPUT)
...
def test_custom_command_errors(sg_pg_conn):
    ...
    with pytest.raises(SplitGraphException) as e:
        execute_commands(load_sgfile('custom_command_dummy.sgfile').replace('DUMMY', 'NOP'), output=OUTPUT)
    assert "Custom command NOP not found in the config!" in str(e.value)

    # Test we raise for commands that can't be imported
    with pytest.raises(SplitGraphException) as e:
        execute_commands(load_sgfile('custom_command_dummy.sgfile').replace('DUMMY', 'BROKEN1'), output=OUTPUT)
    assert "Error loading custom command BROKEN1" in str(e.value)
    with pytest.raises(SplitGraphException) as e:
        execute_commands(load_sgfile('custom_command_dummy.sgfile').replace('DUMMY', 'BROKEN2'), output=OUTPUT)

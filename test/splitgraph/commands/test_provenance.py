...
from test.splitgraph.conftest import OUTPUT, PG_MNT, add_multitag_dataset_to_remote_driver, load_splitfile
...
def test_splitfile_recreate(empty_pg_conn, remote_driver_conn):
    add_multitag_dataset_to_remote_driver(remote_driver_conn)
...
def test_splitfile_recreate_custom_from(empty_pg_conn, remote_driver_conn):
    add_multitag_dataset_to_remote_driver(remote_driver_conn)
    execute_commands(load_splitfile('from_remote.splitfile'), params={'TAG': 'v1'}, output=OUTPUT)
    recreated_commands = image_hash_to_splitfile(OUTPUT, get_current_head(OUTPUT), err_on_end=False)
...
def test_splitfile_incomplete_provenance(empty_pg_conn, remote_driver_conn):

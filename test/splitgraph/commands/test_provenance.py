...
def test_rerun_with_new_version(empty_pg_conn, snapper_conn):
    _add_multitag_dataset_to_snapper(snapper_conn)
    ...
    assert provenance(empty_pg_conn, 'output', output_v1) == \
        [('test_pg_mount', get_tagged_id(snapper_conn, 'test_pg_mount', 'v1'))]
...
def test_rerun_with_from_import(empty_pg_conn, snapper_conn):
    _add_multitag_dataset_to_snapper(snapper_conn)

...
def test_table_changes(include_snap, pg_repo_local):
    ...
    assert pg_repo_local.diff('fruits_copy', image_1=head, image_2=None) is True
    ...
    assert pg_repo_local.diff('fruits_copy', image_1=head, image_2=head_1) is True
...
def test_tagging(pg_repo_local):
    ...
    pg_repo_local.get_image(head).tag('base')
    pg_repo_local.get_image(right).tag('right')

    pg_repo_local.checkout(tag='base')
    assert pg_repo_local.get_head() == head

    pg_repo_local.checkout(tag='right')
    assert pg_repo_local.get_head() == right
...
def test_image_resolution(pg_repo_local):
    head = pg_repo_local.get_head()
    assert pg_repo_local.resolve_image(head[:10]) == head
    assert pg_repo_local.resolve_image('latest') == head
    with pytest.raises(SplitGraphException):
        pg_repo_local.resolve_image('abcdef1234567890abcdef')
    with pytest.raises(SplitGraphException):
        pg_repo_local.resolve_image('some_weird_tag')
